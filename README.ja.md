...
Go言語ベースのモックTODOサーバー。タスク管理のためのREST APIエンドポイントと包括的な認証サポートを提供する。開発環境でOAuth2/OIDCクライアントや様々な認証シナリオをテストするために設計されている。
...
| GET | `/auth/jwks` | JSON Web Key Setを取得 |
| GET/POST | `/auth/register` | ユーザー登録（Webフォーム） |
...
OIDCモードでは、OIDC認証フローとは独立して動作するWebベースのユーザー登録システムが含まれる：

- **登録URL**: `/auth/register`に直接アクセス（OIDCパラメータは不要）
- **OIDCフローから独立**: 登録はサーバーサイド機能であり、OAuth2/OIDC仕様の一部ではない
- **Webインターフェース**: ユーザー名/パスワード登録用のHTMLフォームを提供
- **開発重視**: フロントエンド開発とテスト時のユーザー作成を簡素化するために設計

```bash
# 登録ページに直接アクセス
http://localhost:8080/auth/register

# またはプログラムでユーザーを登録（OIDCモード）
curl -X POST http://localhost:8080/auth/register \
  -H "Content-Type: application/x-www-form-urlencoded" \
  -d "username=testuser&password=testpass"
```

登録後、ユーザーは標準的なOIDC認証フローで認証できます。

**OIDCフローの例:**

1. **ユーザー登録**（オプション）: `/auth/register`でテストユーザーを作成
2. **認可リクエスト**: 適切なパラメータで`/auth/authorize`にユーザーを誘導する
3. **ユーザーログイン**: ユーザーがWebフォームで認証する
4. **認可コード**: サーバーが認可コードでリダイレクトする
5. **トークン交換**: `/auth/token`で認可コードをアクセス/IDトークンと交換する
6. **API アクセス**: アクセストークンを使用して保護されたエンドポイントを呼び出す
